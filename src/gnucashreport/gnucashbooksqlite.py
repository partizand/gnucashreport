...
class GNUCashBookSQLite(GNUCashBook):
    ...
    def read_book(self, filename):
        ...
        self.df_prices[cols.VALUE] = self.df_prices.apply(lambda row: Decimal((row['value_num'] / row['value_denom'])),
                                                          axis=1)
        ...
        self.df_splits[cols.QUANTITY] = self.df_splits.apply(lambda row: Decimal((row['quantity_num'] / row['quantity_denom'])),
                                                          axis=1)
