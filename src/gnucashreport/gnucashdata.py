...
class GNUCashData:
    ...
    def _add_xirr_info2(self):
        ...
        for tr_guid in self.df_splits.index.get_level_values(0):
            ...
            # df_all_assets = df_tr_splits[df_tr_splits[cols.ACCOUNT_TYPE].isin(self.ALL_XIRR_TYPES)]
            # тут нужно добавить всем df_all_assets xirr_value, если у них включено xirr_enable
            ...
            if any(df_tr_splits[cols.ACCOUNT_TYPE].isin(self.STOCK_XIRR_TYPES)):
                ...
                self._add_xirr_value(df_tr_splits, on_types=self.INCEXP_XIRR_TYPES, on_xirr_enable=True, xirr_account=master_guid)
                pass
            else:
                # Проводка без stock
    ...
    #     df_r_splits = df_r_splits[df_r_splits[cols.ACCOUNT_TYPE].isin(self.ALL_XIRR_TYPES)]
    ...
    #     df_r_splits.sort_values(cols.ACCOUNT_TYPE, inplace=True)
    ...
    #     """
    #     # tr_guid = row[cols.TRANSACTION_GUID]
    #     # exclude_guid = row['guid']
    #     df_r_splits = self.df_splits[self.df_splits[cols.TRANSACTION_GUID] == row[cols.TRANSACTION_GUID]]
    #     # df_r_splits.rename(columns={cols.SHORTNAME: 'name2'}, inplace=True)
